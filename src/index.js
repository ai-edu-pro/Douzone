...
function reducer(state=basicState, action){
  ...
  if(action.type === 'ADDITEM'){
      ...
      // let searchIndex = state.findIndex((item)=>{ return item.name === action.payload.id});
      let searchName = state.findIndex((item)=>{ return item.name === action.payload.name});
  
      if(searchName === undefined ){
        let newState = [...state];
        newState[stateIndex].quan++;
        // newState[action.no].quan++;
    ...
    if(newState[action.no].quan <= 0 ) newState[stateIndex].quan= 0;
